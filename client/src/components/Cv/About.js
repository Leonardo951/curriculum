...
class About extends Component {
    ...
    dateBirth = ()=> {
        const date = this.props.curriculumData.dateBirth;
        if(date){
            const day = date.getDate().toString();
            let month = (date.getMonth()+1).toString();
            const yearBirth = date.getFullYear().toString();
            date = day+'/'+month+'/'+yearBirth;
            return date;
        }
    };
    ...
    render() {
        ...
        const { name, jobMain, sex,website, otherMail, nationality, civilStatus, address, city, state, phone, socialNetworks } = this.props.curriculumData;
                                ...
                                <div className="cole s12 fadeInUp animated wow" data-wow-duration="1s" data-wow-delay="0.3s"
                                    ...
                                    <div className="cole s12 w-block shadow-bg pd-0">
                                        <div className="cole m5 s12 about-img parallax-layer al-center pd-50 aa">
                                            ...
                                            <div className="about-name" style={{color: this.props.colorCv}}>{ name }</div>
                                        ...
                                        <div className="cole m7 s12 about-data-wrapper pd-50">
                                            <div className="about-desc pd-0">
                                                ...
                                                <div className="about-data">
                                                    ...
                                                    { sex !== 'notInformed' &&  <div><span>Sexo</span>{ sex }</div> }
                                                    <div><span>Estado Civil</span>{ civilStatus }</div>
                                                    <div><span>Endereço</span>{ address+'/ '+city+'-'+state }</div>
                                                    <div>
                                                        <span>Telefone</span>
                                                        { phone.phoneOne }
                                                        { phone.phoneTwo !== '' && '/ '+phone.phoneTwo }
                                                        { phone.phoneThree !== '' && '/ '+phone.phoneThree }
                                                    </div>
                                                    <div>
                                                        <span>E-mail { otherMail !== '' && 's' }</span>
                                                        { this.props.auth.mail }
                                                        { otherMail !== '' && '/'+otherMail }
                                                    </div>
